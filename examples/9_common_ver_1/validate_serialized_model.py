...
FEATURES = ['PNA', 'SOI', 'SOI_STANDART', 'day_of_year', 'max_Mean_PDSI', 'max_PREC_DAILY', 'max_TAVG_DAILY',
            'max_TMAX_DAILY', 'max_TMIN_DAILY', 'max_WTEQ_DAILY', 'mean_Mean_PDSI', 'mean_PREC_DAILY',
            'mean_TAVG_DAILY', 'mean_TMAX_DAILY', 'mean_TMIN_DAILY', 'mean_WTEQ_DAILY', 'min_Mean_PDSI',
            'min_PREC_DAILY', 'min_TAVG_DAILY', 'min_TMAX_DAILY', 'min_TMIN_DAILY', 'min_WTEQ_DAILY',
            'sum_Mean_PDSI', 'sum_PREC_DAILY', 'sum_TAVG_DAILY', 'sum_TMAX_DAILY', 'sum_TMIN_DAILY', 'sum_WTEQ_DAILY']
...
def collect_features_for_prediction(site_id: str, data_dir: Path, preprocessed_dir: Path, issue_date: str):
    ...
    path_to_snodas = Path(data_dir, 'snodas').resolve()
    ...
    # PREPARE SNODAS
    # snodas_df = collect_snodas_data_for_site(issue_date, aggregation_days_snodas, path_to_snodas, data_dir, preprocessed_dir, site_id)
    # snodas_df = generate_datetime_into_julian(dataframe=snodas_df, datetime_column='datetime',
    #                                           julian_column='julian_datetime', round_julian=3)
    # snodas_features = aggregate_data_for_issue_date(issue_date, int(issue_date.strftime('%j')), snodas_df,
    #                                                 aggregation_days_snodas, 'snodas')

    current_dataset = None
    for df in [snotel_features, telecon_features, pdsi_features]:
...
def validate():
    ...
    validator = ModelValidation(folder_for_plots='common_model_ver_1',  years_to_validate=validation_year)
    ...
    TimeSeriesPlot(validation_year).predicted_time_series(predicted, plots_folder_name='predictions_common_ver_1')
